...
def _win():
    return make_window("u-c", 20, "America/Los_Angeles")
...
def test_registry_resolves_shipped_recipe_and_policy(monkeypatch):
    # The LOCAL registry backend is what this asserts, and `http` is the shipped
    # default, so the backend under test is selected explicitly.
...
def test_reservoir_client_admit_and_amp_replay_match_the_reservoir(tmp_path):
    ...
    para = "y" * 300
    client.admit("u-r", make_window("u-r", 18, "UTC").window_id, "test-recipe-v0",
                 "\n\n".join(f"{para}{i}" for i in range(20)))
    replay = client.sample_replay("u-r", target_chars=10_000, frac=0.3, seed=1,
                                  before_window=make_window("u-r", 20, "UTC").window_id,
                                  source="amp")
...
def test_reservoir_client_rawlog_reads_prior_daylogs(tmp_path):
    ...
    win_prior = make_window("u-r", 18, "UTC")
    ...
    client.admit("u-r", win_prior.window_id, "test-recipe-v0",
                 "amplified\n\nprovenance only")
    replay = client.sample_replay(
        "u-r", target_chars=100_000, frac=0.5, seed=1,
        before_window=make_window("u-r", 20, "UTC").window_id,
...
def test_reservoir_factory_wires_the_daylog_client(monkeypatch):
    monkeypatch.setenv("CONTINUUM_STORAGE_CLIENTS", "local")
...
def test_run_cycle_default_wires_all_three_clients(var_dir):
    ...
    The day-log leg is proved by the REFUSAL: `var_dir` pins the local backends and
    hands the cycle no records, which is the one combination that has no answer, and
    only `day_log_client` raises it. It used to be proved by stubbing
    `fetch_window_records` to `[]` and asserting `skipped_no_data` — which is the
    defect written down as an expectation: an ingest-axis window read on the event
    axis returns nothing, and "nothing" was indistinguishable from a quiet night.
    """
    from app.clients import IngestWindowNotReadable
    from app.cycle import run_cycle
    win = make_window("u-default", 20, "UTC")
    with pytest.raises(IngestWindowNotReadable):
        run_cycle(win)   # no daylog_client, no registry, no recipe, no policy

    # Registry + reservoir still resolve from settings on the same call: give the
    # cycle records in hand (the local client's real job) and it reaches a status.
    result = run_cycle(win, daylog_client=LocalDayLogClient.from_records([]))
