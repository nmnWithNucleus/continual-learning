...
if ! curl -fsS --max-time 2 "${STORAGE_URL}/health" >/dev/null 2>&1; then
  ...
  echo "     day-log, not the window. A minimal valid C2 (every required field of"
  echo "     contracts/c2_processed_record.v0.json), copy-pasteable:"
  echo "       curl -sS -X POST ${STORAGE_URL}/context/records \\"
  echo "         -H 'content-type: application/json' -d '{"
  echo "           \"contract\":\"C2\",\"version\":\"0\",\"record_id\":\"r-demo-1\","
  echo "           \"user_id\":\"${DEMO_USER}\",\"source\":{\"device_id\":\"d1\",\"stream_id\":\"s1\","
  echo "           \"chunk_id\":\"c1\",\"blob_ref\":\"b1\",\"modality\":\"audio\"},"
  echo "           \"t_start\":\"2026-07-27T09:00:00Z\",\"t_end\":\"2026-07-27T09:00:10Z\","
  echo "           \"content\":{\"kind\":\"transcript\",\"text\":\"a demo utterance\"},"
  echo "           \"enrichments\":{\"speakers\":[],\"faces\":[],\"places\":[],\"objects\":[]},"
  echo "           \"pipeline_version\":\"demo-v0\",\"processed_at\":\"2026-07-27T09:00:11Z\"}'"
