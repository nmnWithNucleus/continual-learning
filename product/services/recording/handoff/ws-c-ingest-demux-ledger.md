...
- `app/emitter.py` — per-session in-order worker: for each received segment, demux → per
  ...
  `failed` (visible in the report); `POST /ingest/sessions/{id}/retry` re-enqueues failures.
- `app/ingest_web.py` — the router: segment upload (idempotent on `(session_id, seq)`; sha256
...
## Gap report — `GET /ingest/sessions/{id}/report`
...
"zero silent loss" guarantee, checked end-to-end across both legs. `GET /ingest/sessions`
...
- 2026-07-18 — built as specced: `app/ledger.py` (SQLite WAL, connection-per-call,
  ...
  failures parked for `/retry`), `app/ingest_web.py` (upload idempotent on (session, seq);
...
- 2026-07-18 — **adversarial review round** (multi-agent find → 2-skeptic verify over the
  ...
  (3) *unbounded body*: `/ingest/segments` now streams the body against a cap
