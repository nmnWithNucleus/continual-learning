"""Headless module CLI for one capture (integrator/dev convenience).
...
data-processing, and prints the capture summary as JSON. (No live services run in the
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(prog="python -m app.cli", description="Run one capture.")
...
async def _run(args: argparse.Namespace) -> dict:
    return await capturer.run_capture(
        settings=get_settings(),
