"""The clip captioner wire.
  ...
  * ``vlm`` — the real path: ONE multi-image call against an OpenAI-compatible
    ``/v1/chat/completions``, OCR text injected, replies run through
...
There is no env resolver and no mock module here (L4): the backend is named in the
stage's code-resolved ``Backend``, and mock dialects are client-level fakes in tests —
a ``httpx.MockTransport`` behind ``vlm.make_async_client``, or a canned stage double.
"""
