"""The delta gate: change-map math + the OCR-frame selector.
 ...
 * an ``idle`` flag (no change event fired the whole chunk → the caption uses the
 idle prompt, per the active/idle split in the prompt pack).

WHY binarize-at-full-resolution then area-average then MAX (defect #26): a
...
Production OCR idle/layout thresholds are pinned in ``clipprep``'s ``ClipSettings``;
the anchor accumulator is a documented synthesis, not a measured configuration.
Build to the specified defaults; they get pinned in the stage file.
"""
...
# floor; recalibration may retune it against real capture. A cell at or below the guard
# contributes nothing; above it, the cell's full value accumulates since the last OCR read.
...
def classify(peak: int, spread: int, *, idle_peak: int, layout_peak: int) -> str:
    """Event class from an (accumulated) map's peak/spread. IDLE first: a peak at or
...
def even_spaced_subset(items: list, cap: int) -> list:
    """Keep ``cap`` items evenly spaced across a TIME-SORTED list (the frames.py idiom).
 Rank-free by construction — the choice depends only on position, never on a
...
def floor_time(times: list[float], t_start_epoch: float, floor_s: float) -> float | None:
    """Floor grid, pinned convention (no ``t_prev`` ambiguity): the FIRST analysis
...
def select_ocr_frames(
 ...
 ``Delta.accum`` is the per-delta accumulated peak (the anchor trace). Thresholds
 were calibrated on real screen captures, 2026-07; recalibration requires a new
 probe. Everything is a pure function of the inputs, so two runs are byte-identical.
 """
