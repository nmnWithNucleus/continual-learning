"""OCR post-processing → one self-anchored single line (steps 2/3/5/6,,).
...
The post-processing order, applied verbatim (event selection — step 1's floor grid ∪
change events, capped — is ``clipprep``'s delta gate; here we run everything AFTER the read):
 ...
 a knob; the prior ``VIDEO_PRIVACY_FILTER`` env flag was never read and is dead);
...
# The frozen role vocabulary (step 2 / clip_types.OcrRegion.role).
ROLES = (
...
def assign_role(bbox: tuple[float, float, float, float]) -> str:
 ...
 the parked ``enrichments.text_regions[]``, /§6.2). Deterministic; returns one of
...
def render(
    ...
    *,
    ...
    """Run steps 2-6 over the chunk's ``OcrRead``s and return
