...
1. NAME THE SURFACE. Start from the application or website in focus and the specific view
   inside it. If you cannot identify it, write "an unidentified <kind> window" and move on.
   Never guess a brand.
...
3. NAME WHAT YOU CAN READ, AND NOTHING ELSE. Report the thread, the file, the page or the
   person only when you can read it in these frames. You may quote at most ONE short
   phrase, in double quotes, and only if you can read that exact phrase. NEVER state a
   name, number, price, address or quoted string you cannot read. A separate record carries
   the verbatim on-screen text; your job is the action.
