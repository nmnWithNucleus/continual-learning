...
`/context` writes, same dedup, proven by the unchanged 72 M0 tests. Flip `INGEST_ASYNC=1` to
enable the worker pool; that *retires the `RECORDING_HTTP_TIMEOUT=120` mitigation* (revert
recording's timeout to 30).
