...
13. Ingest processing mode. M0 `/ingest` processes **inline** (pull → ASR → C2 → store inside the request handler, returns `record_id`), so recording's push blocks on ASR latency. **Prod:** `/ingest` should **ACK `202` immediately + enqueue**, processing on a worker pool decoupled from capture cadence; dedup-on-`chunk_id` + `record_id` determinism keep at-least-once safe. Backpressure / dead-letter land with M7.
