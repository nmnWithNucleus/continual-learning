...
Rebuilt for the : the prior backend resolver (`_resolve`/`select`/`version_tag`)
and the legacy ppocr client died with the env-selected seam (the engine is
