...
The PPOCREngine from the prior OCR service (retired at), ported faithfully
...
instead of prior bespoke /ocr, and NO OCR_* env knobs (L4: engine
settings are pinned in code below; a model swap is a code change now, not an
env override).

Engine pins (the prior service's measured operating point, now code constants):
...
/infer contract:
 ...
 as prior did). Invalid base64 / undecodable image -> BackendError
...
Concurrency: prior guarded self._ocr with a per-call lock; here the
...
_THREADS = 4 # ORT intra-op threads (prior measured op point)
...
def _strip_data_uri(b64: str) -> str:
 """Tolerate a `data:image/jpeg;base64,....` prefix (prior decode posture)."""
...
class OcrBackend(ModelBackend):
 """PP-OCR det+rec ONNX on CPU via rapidocr-onnxruntime (prior
 ...
 rapidocr-onnxruntime package (its models/ dir), exactly like prior
