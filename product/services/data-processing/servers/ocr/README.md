...
The prior OCR service's engine (`PPOCREngine` — PP-OCRv4 det+rec ONNX on CPU via
...
- **Wire**: the framework contract (`GET /health` with identity, `POST /infer`
 `{input_b64, codec, params}` → `{ok, result}`), not prior bespoke
...
- **No env knobs (L4)**: prior `OCR_MODE/OCR_EP/OCR_THREADS/OCR_*_MODEL`
...
- **This was built beside the prior OCR service, not migrated from it.** At the Stage
 F cutover the prior process was stopped and its tree removed at ; this server
...
Same det/rec shas prior served — the same bundled ch_PP-OCRv4 ONNX pair.
