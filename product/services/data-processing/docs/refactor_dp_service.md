...
- **L8 — Done-ledger & healing.** The journal's done-row is `chunk_id → {pipeline_version, record_id, stage_status{name→ok|failed|cancelled}, heal_attempts}`. On (re)delivery:
  ...
  4. version matches, holes, budget left → **heal**: full graph re-run (the redelivery body carries the C1 envelope, so no stored envelope is needed), re-POST same `record_id`, upsert replaces holey with fuller; same stage fails again ⇒ `heal_attempts++`; at budget ⇒ holes **permanent**, row done-final, metric fires.
