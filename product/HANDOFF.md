...
| 4 | **C5 shape pin** — inherited constraints, not just docs: a **three**-value status enum, **nullable** `adapter_dir` + `base_model_hash`, and C6 eligibility as a **log replay** (not "latest row wins", or a gate-failed candidate becomes servable). | storage + continuum + inference | Storage's `model_directory` is still the trivial C6 row, so hosting C5 is a build, not a transport swap. Deferred by D19; free while C5's shape is unpinned |
