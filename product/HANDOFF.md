...
| Aspect | File | State |
|---|---|---|
| Engineering | [handoff/engineering.md](handoff/engineering.md) | active — **D18/D19/D20 SHIPPED (2026-07-27):** the storage expansion is built, continuum is cut over to storage's HTTP surface, and the learn fleet runs on it (real capture → `/context` → a nightly to *published* over HTTP, proven live). Suites storage **310** · continuum **262** · recording **144** · DP **788**+21. All eleven accuracy-review items closed and the review file retired into the docs it explained. Next founder acts are follow-ups, not blockers: **E-2**'s retraction primitive (the fleet cutover hit its absence directly), the **D9 observability backbone**, and **E-3(b)**. Earlier: serve-loop v0.0 **closed on real Qwen3-VL-32B**; capture M1 + computer surfaces DONE; DP v1 + HARDENING merged; **D15 continuum kickoff → LEARN LOOP CLOSED (2026-07-25): Morpheus port parity-proven, M0 (32B adapter → C5 → vLLM), Phase-3 dogfood proves the real recording→DP→storage→continuum path carries it (PIPELINE SOUND).** Next founder acts: **storage/C10 board session** (expansion + C10 evolution — now carrying a fourth expansion row, the per-user profile owning `home_tz`, per **D17** 2026-07-26: timezone ownership decided, review item O-1 closed, C1/C2 untouched) |
